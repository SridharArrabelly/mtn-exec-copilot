...
│                         │         │                             │         │     Service      │ agent_  │      (avatar-forge agent)    │
...
2. **Configure environment (`.env` in project root):**
   ...
   - `SEARCH_INDEX_NAME` - **Required.** Index name to create/update (e.g. `meeting-index`)
...
5. **Upload** — pushes the chunks + vectors into the index, which is configured for **hybrid search** (BM25 + HNSW/cosine) with a **semantic configuration** (`default-semantic`) for L2 re-ranking.
...
#      - Environment name (azd env name, e.g. "avatar-dev")
#      - Resource Group name (the RG that will be created, e.g. "rg-avatar-dev")
azd up
```
...
azd init     # prompts for env name (e.g. avatar-dev) and picks template files

# 3. Tell azd which subscription / region / RG to use
azd env set AZURE_SUBSCRIPTION_ID     <sub-guid>
azd env set AZURE_LOCATION            eastus2
azd env set AZURE_RESOURCE_GROUP_NAME rg-avatar-dev

# 4. Point at the EXISTING Foundry account + project
azd env set EXISTING_FOUNDRY_ACCOUNT_NAME     my-foundry-prod
azd env set EXISTING_FOUNDRY_RESOURCE_GROUP   rg-shared-ai
azd env set EXISTING_FOUNDRY_PROJECT_ENDPOINT https://my-foundry-prod.services.ai.azure.com/api/projects/avatar-forge

# 5. Point at the EXISTING AI Search service + index
azd env set EXISTING_SEARCH_SERVICE_NAME   my-search-prod
azd env set EXISTING_SEARCH_RESOURCE_GROUP rg-shared-ai
azd env set EXISTING_SEARCH_INDEX_NAME     meeting-index
...
|---|---|---|
| Resource Group (`rg-avatar-dev`) | ✅ Created | From your `AZURE_RESOURCE_GROUP_NAME` |
...
| `SEARCH_INDEX_NAME`       | `meeting-index`                  | AI Search index name                     |
